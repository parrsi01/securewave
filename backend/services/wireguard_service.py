...
class WireGuardService:
    def __init__(self, firewall: FirewallManager | None = None) -> None:
        ...
        self.source_cidr = os.getenv("WG_CIDR", "10.8.0.0/24")
        self.egress_iface = os.getenv("WG_EGRESS_IFACE", "").strip() or self._detect_egress_iface()
        self.tunnel_iface = os.getenv("WG_TUNNEL_IFACE", "wg0").strip() or "wg0"
    ...
    def setup_network_state(self) -> None:
        with routing_manager.network_lock():
            routing_manager.setup_protocol("wireguard", self.source_cidr, self.tunnel_iface)
            self.firewall.setup_protocol_nat("wireguard", self.source_cidr, self.egress_iface)

    def teardown_network_state(self) -> None:
        with routing_manager.network_lock():
            self.firewall.teardown_protocol_nat("wireguard", self.source_cidr, self.egress_iface)
            routing_manager.teardown_protocol("wireguard", self.tunnel_iface)
    ...
    def start_meter(self, user_id: int, session_id: str | None = None) -> dict:
        started = get_traffic_manager().start_meter(
            user_id=user_id, protocol="wireguard", session_id=session_id, iface_hint=self.tunnel_iface
        )
    ...
    def apply_shaping(self, user_id: int, tier: str, session_id: str) -> dict:
        return get_traffic_shaper().apply_for_session(user_id, "wireguard", tier, session_id, self.tunnel_iface)
