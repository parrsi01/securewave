...
if run_one "dns_leak_test" "$ROOT_DIR/sandbox/leak_tests/dns_leak_test.py"; then
  ((pass_count+=1))
else
  ((fail_count+=1))
fi

if run_one "ipv6_leak_test" "$ROOT_DIR/sandbox/leak_tests/ipv6_leak_test.py"; then
  ((pass_count+=1))
else
  ((fail_count+=1))
fi

if run_one "route_table_test" "$ROOT_DIR/sandbox/leak_tests/route_table_test.py"; then
  ((pass_count+=1))
else
  ((fail_count+=1))
fi

if run_one "interface_flap_test" "$ROOT_DIR/sandbox/leak_tests/interface_flap_test.py"; then
  ((pass_count+=1))
else
  ((fail_count+=1))
fi
...
if "$PYTHON_BIN" "$ROOT_DIR/sandbox/leak_tests/enforce_thresholds.py" \
  --leak-dir "$OUT_DIR" \
  --thresholds "$ROOT_DIR/leak/leak_thresholds.json" \
