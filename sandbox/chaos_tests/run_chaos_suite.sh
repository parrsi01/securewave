...
if run_one "network_drop" "$ROOT_DIR/sandbox/chaos_tests/network_drop.py"; then
  ((pass_count+=1))
else
  ((fail_count+=1))
fi

if run_one "db_disconnect" "$ROOT_DIR/sandbox/chaos_tests/db_disconnect.py"; then
  ((pass_count+=1))
else
  ((fail_count+=1))
fi

if run_one "jwt_replay_attack" "$ROOT_DIR/sandbox/chaos_tests/jwt_replay_attack.py"; then
  ((pass_count+=1))
else
  ((fail_count+=1))
fi
...
if "$PYTHON_BIN" "$ROOT_DIR/sandbox/chaos_tests/enforce_thresholds.py" \
  --chaos-dir "$OUT_DIR" \
  --thresholds "$ROOT_DIR/chaos/chaos_thresholds.json" \
