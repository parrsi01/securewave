...
def test_fresh_migration_head_is_repeatable_and_covers_runtime_models(monkeypatch, tmp_path):
    ...
    assert {"users", "vpn_servers", "vpn_connections", "vpn_usage_events", "invoices"} <= tables
    user_columns = {
...
def test_legacy_orm_audit_shape_upgrades_without_duplicate_columns(monkeypatch, tmp_path):
    ...
    from models import audit_log, email_log, gdpr, invoice, subscription, support_ticket, usage_analytics, user, vpn_connection, vpn_demo_session, vpn_server, vpn_usage_event, wireguard_peer  # noqa: F401
