...
def test_linux_x64_deb_is_withheld_until_current_runtime_evidence():
    entries = downloads._build_download_entries()
    linux_x64_deb = next(
        entry for entry in entries
    ...
    assert linux_x64_deb.evidence_url == "https://github.com/parrsi01/securewave/actions/runs/29036573515"
    assert linux_x64_deb.checksum_sha256 == (
        "f2718810c7dea6e2c298c159f25d904321423ab3a359c1d1428b3e824d7b4d92"
    )
    assert "current source" in (linux_x64_deb.notes or "")
...
def test_invalid_manifest_structure_uses_safe_fallback(monkeypatch, tmp_path, client):
    ...
    assert withheld["url"] == "#"
    assert withheld["evidence_url"] == "https://github.com/parrsi01/securewave/actions/runs/29036573515"
