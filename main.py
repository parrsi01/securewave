...
This module bootstraps the SecureWave FastAPI application and its startup, middleware, and route registration.
See `docs/code-explanations/main.md` for the generated line-range walkthrough.
"""
...
from services.privileged_netops_client import PrivilegedNetopsError
...
async def lifespan(app: FastAPI):
    ...
    validate_privileged_netops_runtime(logger)
...
def validate_privileged_netops_runtime(logger: logging.Logger) -> None:
    if os.getenv("TESTING", "").lower() == "true":
        return
    service = PrivilegedNetworkService()
    if not service.is_enabled():
        logger.info("Privileged netops daemon disabled")
        return
    try:
        status = service.health_ping()
        logger.info("Privileged netops daemon healthy: %s", status)
    except PrivilegedNetopsError as exc:
        if service.is_required():
            raise RuntimeError(f"Privileged netops daemon required but unavailable: {exc}") from exc
        logger.warning(
            "Privileged netops daemon unavailable; backend may fall back to legacy network mutations: %s",
            exc,
        )
