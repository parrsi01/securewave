...
run_step "live_e2e_validate" \
  "$PYTHON_BIN" "$ROOT_DIR/dev_tools/sandbox/live_validation/live_e2e_validate.py" \
  --output-dir "$OUT_DIR" \
...
run_step "system_audit_before" \
  "$PYTHON_BIN" "$ROOT_DIR/dev_tools/sandbox/system_audit/system_audit_probe.py" \
  --output-dir "$ROOT_DIR/artifacts/system_audit" \
  --api-base-url "${LIVE_API_BASE_URL:?LIVE_API_BASE_URL is required}" \
  --label "before_live_validation"

run_step "geo_latency_probe" \
  "$PYTHON_BIN" "$ROOT_DIR/dev_tools/sandbox/live_validation/geo_latency_probe.py" \
  --output-dir "$OUT_DIR" \
  --config "$ROOT_DIR/dev_tools/sandbox/live_validation/geo_targets.json"

run_step "network_failure_cases" \
  "$PYTHON_BIN" "$ROOT_DIR/dev_tools/sandbox/live_validation/network_failure_cases.py" \
  --output-dir "$OUT_DIR" \
  --api-base-url "${LIVE_API_BASE_URL:?LIVE_API_BASE_URL is required}" \
  "${strict_flag[@]}" \
  "${fault_execute_flag[@]}"
...
run_step "readiness_report" \
  "$PYTHON_BIN" "$ROOT_DIR/dev_tools/sandbox/live_validation/reporting.py" --output-dir "$OUT_DIR"
