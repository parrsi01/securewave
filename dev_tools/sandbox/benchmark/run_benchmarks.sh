...
if run_one "ping_latency" "$ROOT_DIR/dev_tools/sandbox/benchmark/ping_latency.py" --output-dir "$OUT_DIR"; then
  ((pass_count+=1))
else
  ((fail_count+=1))
fi

if run_one "jitter_packet_loss" "$ROOT_DIR/dev_tools/sandbox/benchmark/jitter_packet_loss.py" --output-dir "$OUT_DIR"; then
  ((pass_count+=1))
else
  ((fail_count+=1))
fi

if run_one "handshake_performance" "$ROOT_DIR/dev_tools/sandbox/benchmark/handshake_performance.py" --output-dir "$OUT_DIR"; then
  ((pass_count+=1))
else
  ((fail_count+=1))
fi

if run_one "throughput_test" "$ROOT_DIR/dev_tools/sandbox/benchmark/throughput_test.py" --output-dir "$OUT_DIR"; then
  ((pass_count+=1))
else
  ((fail_count+=1))
fi

if run_one "competitor_probe" "$ROOT_DIR/dev_tools/sandbox/benchmark/competitor_probe.py" \
  --output-dir "$OUT_DIR" \
...
enforce_thresholds() {
  ...
  if "$PYTHON_BIN" "$ROOT_DIR/dev_tools/sandbox/benchmark/enforce_thresholds.py" \
    --benchmark-dir "$OUT_DIR" \
    --thresholds "$ROOT_DIR/dev_tools/benchmarks/thresholds.json" \
