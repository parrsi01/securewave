...
The macOS UI demo app has a Mac-only packaging command and public download
slots, but the actual `.app` zip must be produced on macOS.
...
- macOS UI demo zip after the Mac-side package command is run and the generated file is published.
...
SecureWave should not be described as a production-scale SaaS, multi-platform VPN, or paid subscription product currently live unless separate release evidence exists. Public messaging should describe the current truth: Linux desktop free release candidate now, Apple/iOS packaging in Mac handoff, macOS UI demo packaging pending Mac build, Premium coming soon, and additional platforms/protocols gated by future validation.
