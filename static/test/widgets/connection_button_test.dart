...
class _KillSwitchVpnStateNotifier extends VpnStateNotifier {
  _KillSwitchVpnStateNotifier(super.ref) {
    state = const VpnState(
...
class _ReconnectPendingVpnStateNotifier extends VpnStateNotifier {
  _ReconnectPendingVpnStateNotifier(super.ref) {
    state = const VpnState(
      status: VpnStatus.disconnected,
      desiredOn: true,
      reconnectPending: true,
      reconnectReason: 'Network restored. Reconnecting the protected tunnel.',
    );
  }
}
...
void main() {
  ...
  group('ConnectionRing', () {
    ...
    testWidgets('renders kill switch banner when traffic is blocked',
        (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            ...
            vpnStateProvider
                .overrideWith((ref) => _KillSwitchVpnStateNotifier(ref)),
            serversProvider.overrideWith(
              (ref) async => const <ServerRegion>[
                ServerRegion(
                  id: 'kill-switch-test',
    ...
    testWidgets('renders reconnect banner while automatic recovery is pending',
        (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            ...
            vpnStateProvider
                .overrideWith((ref) => _ReconnectPendingVpnStateNotifier(ref)),
            serversProvider.overrideWith(
              (ref) async => const <ServerRegion>[
                ServerRegion(
                  id: 'reconnect-test',
      ...
      expect(
        find.text('Network restored. Reconnecting the protected tunnel.'),
        findsOneWidget,
      );
    });
