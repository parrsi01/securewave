...
class FakeApiClient extends ApiClient {
  FakeApiClient({
    ...
    this.resolvedRegionId,
    this.resolvedRegionReason = 'auto_failover',
  ...
  final String resolvedRegionReason;

  int profileFetchCalls = 0;
  int notifyConnectedCalls = 0;
  int notifyDisconnectedCalls = 0;
  int reinitializeCalls = 0;
  final List<String> reinitializeReasons = <String>[];
  int resolveRegionCalls = 0;
...
''',
  ...
  @override
  Future<void> reinitializeControlPlane({required String reason}) async {
    reinitializeCalls += 1;
    reinitializeReasons.add(reason);
    await super.reinitializeControlPlane(reason: reason);
  }
  ...
  Future<VpnResolvedRegion> resolveRegion({
    ...
    if (resolvedRegionId == null || resolvedRegionId!.trim().isEmpty) {
      return super.resolveRegion(
        protocol: protocol,
