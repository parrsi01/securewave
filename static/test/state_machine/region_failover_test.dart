...
class _FailoverApiClient extends FakeApiClient {
  ...
  @override
  String get resolvedRegionId => _resolvedRegionId;
...
class _ConnectFailoverService implements VpnService {
  _ConnectFailoverService();
  ...
  Future<VpnStatus> connect({
    ...
    if (connectCalls == 1) {
      ...
      throw VpnServiceException(
        'connect_incomplete',
        'Primary region did not reach connected state.',
      );
    }
...
void main() {
  ...
  test('auto-failover can resolve when region is not manually pinned',
      () async {
    final config = testAppConfig();
    final service = ControlledVpnService(nativeAvailable: true);
    final api = _FailoverApiClient(
  ...
  test('connect-time tunnel failure retries once through fallback region',
      () async {
    final config = testAppConfig();
    final service = _ConnectFailoverService();
    final api = _FailoverApiClient(
      config: config,
      resolvedRegionId: 'fallback-up',
    );
