...
rotating_file_sink<Mutex>::calc_filename(const filename_t& filename,
                                         std::size_t index)
{
    if (index == 0U)
    {
...
template <typename Mutex> std::size_t rotating_file_sink<Mutex>::get_max_files()
{
    std::lock_guard<Mutex> lock(base_sink<Mutex>::mutex_);
    return max_files_;
}
