...
struct kafka_sink_config
{
    ...
    kafka_sink_config(std::string addr,
                      std::string topic,
...
template <typename Mutex> class kafka_sink : public base_sink<Mutex>
{
public:
    kafka_sink(kafka_sink_config config) : config_{std::move(config)}
    {
        try
        {
            ...
            {
                throw_spdlog_ex(
                    fmt_lib::format("conf set bootstrap.servers failed err:{}",
                                    errstr));
            }
