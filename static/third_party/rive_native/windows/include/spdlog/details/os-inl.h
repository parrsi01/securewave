...
SPDLOG_INLINE bool fopen_s(FILE** fp,
                           ...
                           const filename_t& mode)
{
    ...
    *fp = ::_fsopen((filename.c_str()), mode.c_str(), _SH_DENYNO);
    ...
    const int fd = ::open((filename.c_str()),
                          O_CREAT | O_WRONLY | O_CLOEXEC | mode_flag,
                          mode_t(0644));
    if (fd == -1)
    {
        return true;
...
#define SYS_gettid __NR_gettid
    ...
    int retval = pthread_getthrds_np(&pt,
...
SPDLOG_INLINE void wstr_to_utf8buf(wstring_view_t wstr, memory_buf_t& target)
{
    ...
    {
        result_size = ::WideCharToMultiByte(CP_UTF8,
                                            0,
                                            wstr.data(),
                                            wstr_size,
                                            NULL,
    ...
    throw_spdlog_ex(
        fmt_lib::format("WideCharToMultiByte failed. Last error: {}",
                        ::GetLastError()));
}
...
SPDLOG_INLINE void utf8_to_wstrbuf(string_view_t str, wmemory_buf_t& target)
{
    ...
    throw_spdlog_ex(
        fmt_lib::format("MultiByteToWideChar failed. Last error: {}",
                        ::GetLastError()));
}
