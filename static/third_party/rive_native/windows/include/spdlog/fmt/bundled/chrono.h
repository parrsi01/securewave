...
auto duration_cast(std::chrono::duration<FromRep, FromPeriod> from) -> To
{
    ...
    using common_rep = typename std::
        common_type<FromRep, typename To::rep, decltype(factor::num)>::type;
...
inline void write_digit2_separated(char* buf,
                                   unsigned a,
                                   unsigned b,
                                   unsigned c,
                                   char sep)
...
template <typename Period> FMT_CONSTEXPR inline auto get_units() -> const char*
{
...
template <typename T, typename = void> struct has_tm_gmtoff : std::false_type
{};
template <typename T>
...
public:
    ...
    constexpr year_month_day(const year& y,
                             const month& m,
...
public:
        ...
        -> decltype(ctx.out())
    {
        ...
        {
            auto f =
                detail::duration_formatter<Char, Rep, Period>(out,
                                                              d,
                                                              ctx.locale());
...
protected:
                   ...
                   const Duration* subsecs) const -> decltype(ctx.out())
    {
        ...
        auto w =
            detail::tm_writer<basic_appender<Char>, Char, Duration>(loc,
                                                                    out,
                                                                    tm,
                                                                    subsecs);
...
struct formatter<local_time<Duration>, Char> : private formatter<std::tm, Char>
{
        ...
        -> decltype(ctx.out())
    {
        ...
        auto subsecs = detail::duration_cast<Duration>(time_since_epoch -
                                                       seconds_since_epoch);
