...
rotate_log_if_needed() {
  ...
  if [[ -f "$LOG_FILE" ]] && [[ "$(stat -c%s "$LOG_FILE")" -gt 5242880 ]]; then
    mv "$LOG_FILE" "${LOG_DIR}/long_run_test_$(date +%s).json"
  fi
}
...
append_cycle() {
  ...
  entry=$(
    printf '{'
    printf '"type":"cycle",'
    printf '"cycle":%s,' "$cycle_index"
    printf '"started_at":"%s",' "$(json_escape "$started_at")"
    printf '"completed":%s,' "$completed"
    printf '"reconnect_ms":%s,' "$reconnect_ms"
    printf '"handshake_age_seconds":%s,' "$handshake_age"
    printf '"route_consistent":%s,' "$route_ok"
    printf '"cleanup_clean":%s,' "$cleanup_ok"
    printf '"ping_ok":%s,' "$ping_ok"
    printf '"public_ip":"%s",' "$(json_escape "$public_ip")"
    printf '"failure_category":"%s",' "$(json_escape "$failure_category")"
    printf '"failure_detail":"%s"' "$(json_escape "$failure_detail")"
    printf '}'
  )
  write_log_entry "$entry"
  echo "[LOG] cycle=$cycle_index written"
}

append_summary() {
  local ended_at="$1"
  local success_rate="$2"
  local avg_reconnect_ms="$3"
  local entry
  entry=$(
    printf '{'
    printf '"type":"summary",'
    ...
    printf '"cycles_completed":%s,' "$TOTAL_CYCLES"
    printf '"successful_cycles":%s,' "$SUCCESS_CYCLES"
    printf '"success_rate":%s,' "$success_rate"
    printf '"avg_reconnect_ms":%s,' "$avg_reconnect_ms"
    printf '"failure_categories":{'
    printf '"connect_failed":%s,' "$CONNECT_FAILURES"
    printf '"handshake_stale":%s,' "$HANDSHAKE_FAILURES"
    printf '"route_inconsistent":%s,' "$ROUTE_FAILURES"
    printf '"cleanup_residue":%s,' "$CLEANUP_FAILURES"
    printf '"traffic_probe_failed":%s' "$TRAFFIC_FAILURES"
    printf '}'
    printf '}'
  )
  write_log_entry "$entry"
...
append_summary "$ENDED_AT" "$SUCCESS_RATE" "$AVG_RECONNECT_MS"
