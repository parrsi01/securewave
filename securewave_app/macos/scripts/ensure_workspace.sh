#!/bin/bash
# ------------------------------------------------------------------
# ensure_workspace.sh (macOS)
# Xcode pre-action / build-guard script.
#
# PURPOSE: Guarantee that ALL macOS builds go through Runner.xcworkspace,
# never through Runner.xcodeproj directly. Opening the .xcodeproj excludes
# CocoaPods-managed dependencies and causes cryptic errors.
# ------------------------------------------------------------------
set -euo pipefail
...
if [[ -n "${WORKSPACE_PATH:-}" ]]; then
  if [[ "${WORKSPACE_PATH}" != *"Runner.xcworkspace"* ]]; then
    ...
    echo "  2. Open: $MACOS_DIR/Runner.xcworkspace"
    echo "  3. Build again."
    echo "================================================================"
    echo ""
    exit 1
...
if [[ -n "${PROJECT_FILE_PATH:-}" ]] && [[ "${PROJECT_FILE_PATH}" == *"Runner.xcodeproj"* ]] && ( [[ -z "${WORKSPACE_DIR:-}" ]] || [[ "${WORKSPACE_DIR}" == *".xcodeproj"* ]] ); then
  ...
  echo "  are included in the build."
  echo ""
  echo "  HOW TO FIX:"
  echo "  1. Close Xcode."
  echo "  2. Open: $MACOS_DIR/Runner.xcworkspace"
...
if [[ ! -d "$MACOS_DIR/Runner.xcworkspace" ]]; then
  echo ""
  echo "================================================================"
  echo "ERROR: Runner.xcworkspace not found at:"
  echo "  $MACOS_DIR/Runner.xcworkspace"
  echo ""
  echo "  This usually means 'pod install' has not been run."
  echo ""
  echo "  HOW TO FIX:"
  echo "    cd $MACOS_DIR && pod install"
  echo "================================================================"
  echo ""
  exit 1
fi

if [[ ! -f "$MACOS_DIR/Runner.xcworkspace/contents.xcworkspacedata" ]]; then
  echo ""
  echo "================================================================"
  echo "ERROR: Runner.xcworkspace exists but is corrupt."
  echo ""
  echo "  HOW TO FIX:"
  echo "    rm -rf $MACOS_DIR/Runner.xcworkspace"
  echo "    cd $MACOS_DIR && pod install"
  echo "================================================================"
  echo ""
  exit 1
...
WORKSPACE_DATA="$MACOS_DIR/Runner.xcworkspace/contents.xcworkspacedata"

# ------- 4. Workspace must include CocoaPods (Pods.xcodeproj) -------
if [[ ! -d "$MACOS_DIR/Pods" ]]; then
  echo ""
  echo "================================================================"
  echo "ERROR: CocoaPods not installed (missing macos/Pods)."
  echo ""
...
if [[ ! -d "$MACOS_DIR/Pods/Pods.xcodeproj" ]]; then
  echo ""
  echo "================================================================"
  echo "ERROR: CocoaPods project missing: macos/Pods/Pods.xcodeproj"
  echo ""
  echo "  HOW TO FIX:"
  echo "    cd $MACOS_DIR && pod install"
  echo "================================================================"
