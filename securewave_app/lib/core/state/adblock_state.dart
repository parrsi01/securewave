...
class AdblockController extends StateNotifier<AdblockState> {
  ...
  Future<void> load() async {
    ...
    if (totalRules == 0) {
      AppLogger.info('Adblock: loading from cache');
      final cachedCount = await _engine.loadCache();
      if (cachedCount != null && cachedCount > 0) {
        await _persistRules(cachedCount);
        AppLogger.info('Adblock: loaded $cachedCount rules from cache');
        return;
      }
    }

    // Fall back to asset bundle if cache is empty
    if (state.totalRules == 0) {
      AppLogger.info('Adblock: loading fallback from assets');
      await _loadFallback();
    }
  }
