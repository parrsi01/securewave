...
class VpnStateNotifier extends StateNotifier<VpnState> {
  ...
  VpnStateNotifier(this._ref, {VpnStateMachineConfig? config})
      : _config = config ?? _ref.read(vpnStateMachineConfigProvider),
        super(VpnState(
          status: isMockVpn
              ...
              : _ref.read(vpnServiceProvider).getStatus(),
        )) {
    _lastAdapterStatus = state.status;
    _adapterStatusSubscription =
        _ref.read(vpnAdapterProvider).statusStream().listen(
      (status) {
        _lastAdapterStatus = status;
      },
      onError: (Object error, StackTrace stackTrace) {
        AppLogger.warning('[VPN_SM] {"event":"adapter_status_stream_failed"}');
        AppLogger.error(
          'VPN adapter status stream failed',
          error: error,
    ...
    _ref.listen<bool>(
      ...
      (previous, next) {
        ...
        if (!next) {
          if (state.desiredOn || _statusHasActiveTunnel(state.status)) {
  ...
  late final HealthMonitorService _healthMonitor = HealthMonitorService(
    sample: _sampleHealthSnapshot,
    onIssue: _handleHealthMonitorIssue,
    onRecovered: _handleHealthRecovered,
  );

  StreamSubscription<VpnStatus>? _adapterStatusSubscription;
  Timer? _rateTimer;
  ...
  VpnStatus _lastAdapterStatus = VpnStatus.disconnected;
  int _recoveryAttemptCount = 0;
  int _recoveryAttemptTotal = 0;
  ...
  void _debugTrace(String message) {
    AppLogger.debug(message, tag: 'SecureWave.VPN');
  }
  ...
  Future<void> _syncNativeStatus() async {
    if (isMockVpn) {
      _applyRuntimeStatus(_lastAdapterStatus, source: 'mock_adapter');
      return;
    }
    ...
    try {
      ...
      _applyRuntimeStatus(next, source: 'native');
  ...
  void _applyRuntimeStatus(VpnStatus next, {required String source}) {
    _debugTrace(
      '[VPN_DIAG] _syncNativeStatus source=$source result=${next.name} '
      'current=${state.status.name} desiredOn=${state.desiredOn}',
    );
    if (!mounted) return;
    final shouldMirrorIntent =
        _statusIsConnectedLike(next) || _statusIsConnectInFlight(next);
    ...
    _updateReadiness(
      authenticated: _ref.read(authSessionProvider).isAuthenticated
          ? VpnReadinessGateState.ready
          : VpnReadinessGateState.notReady,
      tunnelUp: _statusIsConnectedLike(next)
          ? VpnReadinessGateState.ready
          : VpnReadinessGateState.notReady,
      clearLastErrorCode:
          _statusIsConnectedLike(next) || next == VpnStatus.disconnected,
    );
    if (_statusIsConnectedLike(next)) {
  ...
  Future<void> _attemptAutoConnect({
    ...
    if (_ref.read(deferPostAuthAutoConnectProvider)) {
      AppLogger.info(
        '[VPN_SM] {"event":"auto_connect_deferred","reason":"linux_ui_automation","trigger":"$reason","status":"${state.status.name}","desiredOn":${state.desiredOn}}',
      );
      return;
    }
    final isActive = state.desiredOn || _statusHasActiveTunnel(state.status);
    if (isActive) {
      AppLogger.info(
  ...
  Future<void> switchServer(String? serverId) async {
    ...
    final shouldReconnect =
        state.desiredOn || _statusHasActiveTunnel(state.status);
    if (!shouldReconnect) return;

    AppLogger.info(
      '[VPN_SM] {"event":"server_switch_reconnect","from":"${previousServer ?? ""}","to":"$serverId"}',
    );
    if (_statusHasActiveTunnel(state.status)) {
      await disconnect();
      if (!mounted || _disposed) return;
    }
    await connect();
  ...
  Future<void> connect() async {
    ...
    final connectInFlight = _statusIsConnectInFlight(state.status) ||
    ...
    if (_lastConnectRequestedAt != null &&
        ...
        state.desiredOn) {
      if (kDebugMode) {
        _debugTrace('[VPN_DIAG] connect() debounced');
      }
      return;
    }
    ...
    if (state.desiredOn &&
        (_statusHasActiveTunnel(state.status) ||
            _activeOperation?.action == _VpnOperationAction.connect)) {
      if (kDebugMode) {
        _debugTrace('[VPN_DIAG] connect() ignored (already desired on)');
      }
      return;
    }
    if (!_validateRequestedTransition(
      ...
      trigger: VpnTransitionTrigger.userConnectRequested,
      message:
          'Connect requested from ${state.status.name}, but only DISCONNECTED/RECONNECTING -> CONNECTING is allowed.',
    )) {
      return;
    }
  ...
  Future<void> disconnect() async {
    ...
    if (_lastDisconnectRequestedAt != null &&
        ...
        !state.desiredOn) {
      if (kDebugMode) {
        _debugTrace('[VPN_DIAG] disconnect() debounced');
      }
      return;
    }
    ...
    if (!state.desiredOn &&
        (state.status == VpnStatus.disconnected ||
            ...
            _activeOperation?.action == _VpnOperationAction.disconnect)) {
      if (kDebugMode) {
        _debugTrace('[VPN_DIAG] disconnect() ignored (already desired off)');
      }
      return;
    }
    if (!_validateRequestedTransition(
      ...
      trigger: VpnTransitionTrigger.userDisconnectRequested,
      message:
          'Disconnect requested from ${state.status.name}, but only CONNECTED/DEGRADED/RECONNECTING -> DISCONNECTING is allowed.',
    )) {
      return;
    }
    if (kDebugMode) {
  ...
  Future<void> handleConnectivityChange({required bool hasNetwork}) async {
    ...
    if (!hasNetwork) {
      ...
      if (!_statusIsConnectedLike(state.status)) return;
  ...
  Future<void> handleNetworkPathChange({
    ...
    if (!state.desiredOn || !_statusIsConnectedLike(state.status)) {
      return;
    }
  ...
  void resumeRateUpdates() {
    if (_statusIsConnectedLike(state.status) && _rateTimer == null) {
      _startRateSimulation();
    }
  }
  ...
  Future<void> _performSafeShutdown() async {
    ...
    final service = _ref.read(vpnServiceProvider);
    final shouldDisconnect = _statusHasActiveTunnel(state.status) ||
  ...
  void _setDesiredOnInternal(bool value, {required String source}) {
    ...
    if (previous && !value && kDebugMode) {
      _debugTrace('[VPN_DIAG] desiredOn flipped TRUE→FALSE source=$source');
    }
  ...
  Future<void> _requestReconcile() async {
    ...
    try {
      ...
      _debugTrace('[VPN_DIAG] reconcile loop exception | preserving desiredOn');
      state = state.copyWith(
        isBusy: false,
        errorKind: VpnErrorKind.backendError,
  ...
  void _recoverToDisconnected({
    ...
    final target =
        scheduleReconnect ? VpnStatus.reconnecting : VpnStatus.disconnected;
    if (current != target) {
      _transitionTo(
        target,
        trigger: trigger,
  ...
  bool _statusHasActiveTunnel(VpnStatus status) {
  ...
  bool _statusIsConnectedLike(VpnStatus status) {
    return status == VpnStatus.connected || status == VpnStatus.degraded;
  }

  bool _statusIsConnectInFlight(VpnStatus status) {
    return status == VpnStatus.connecting ||
        status == VpnStatus.verifying ||
        status == VpnStatus.reconnecting;
  }

  bool _statusSupportsHealthMonitoring(VpnStatus status) {
    return status == VpnStatus.connected || status == VpnStatus.degraded;
  }
  ...
  void _startHealthMonitor() {
    ...
    final active = state.status == VpnStatus.connected ||
        state.status == VpnStatus.degraded;
    if (!active || _healthMonitor.isRunning) {
      return;
    }
    _safeFireAndForget(
      _healthMonitor.start(),
      context: 'health_monitor_start',
    );
  }

  void _stopHealthMonitor() {
    if (!_healthMonitor.isRunning) {
      return;
    }
    _safeFireAndForget(
      _healthMonitor.stop(),
      context: 'health_monitor_stop',
    );
  }
  ...
  Future<void> _handleHealthMonitorIssue(VpnHealthIssue issue) async {
    ...
    if (issue.type == VpnHealthFailureType.softFailure) {
      if (state.status == VpnStatus.connected) {
        _transitionTo(
          VpnStatus.degraded,
          trigger: VpnTransitionTrigger.healthMonitorDegraded,
        );
      }
    ...
    await _restartTunnelAfterRuntimeChange(
      ...
      source: 'health_${issue.type.name}',
      reason: issue.reason,
  ...
  Future<void> _handleHealthRecovered(VpnHealthSnapshot snapshot) async {
    ...
    if (state.status == VpnStatus.degraded) {
      _transitionTo(
        VpnStatus.connected,
        trigger: VpnTransitionTrigger.healthMonitorRecovered,
      );
      state = state.copyWith(clearReconnect: true);
  ...
  Future<void> _restartTunnelAfterRuntimeChange({
    ...
    required String reason,
    required VpnHealthFailureType failureType,
  }) async {
    if (!mounted || _disposed) return;
    ...
    if (state.status != VpnStatus.connected &&
        ...
        state.status != VpnStatus.disconnected &&
        state.status != VpnStatus.error &&
        state.status != VpnStatus.verifying &&
    ...
    final service = _ref.read(vpnServiceProvider);
    try {
      final backoff = _scheduleRecoveryBackoff(
        failureType: failureType,
        source: source,
      );
      AppLogger.warning(
        '[VPN_SM] {"event":"runtime_recovery_requested","source":"$source","status":"${state.status.name}","failure_type":"${failureType.name}","backoff_ms":${backoff.inMilliseconds}}',
      );
      _stopHealthMonitor();
      _stopRateSimulation();
  ...
  Duration _currentRecoveryBackoff() {
    ...
    final index = _recoveryAttemptCount <= 0 ? 0 : _recoveryAttemptCount - 1;
    return sequence[index.clamp(0, sequence.length - 1)];
  }

  Duration _scheduleRecoveryBackoff({
    required VpnHealthFailureType failureType,
    required String source,
  }) {
    _recoveryAttemptCount += 1;
    _recoveryAttemptTotal += 1;
    _lastRecoveryFailureType = failureType;
    final delay = _currentRecoveryBackoff();
    _scheduledReconnectAt = DateTime.now().add(delay);
    AppLogger.info(
      '[VPN_SM] {"event":"recovery_backoff_scheduled","source":"$source","failure_type":"${failureType.name}","attempt":$_recoveryAttemptCount,"delay_ms":${delay.inMilliseconds}}',
    );
    return delay;
  }

  void _recordRecoverySuccess({required String source}) {
    _recoverySuccessCount += 1;
    final successRate = _recoveryAttemptTotal == 0
        ? 1.0
        : _recoverySuccessCount / _recoveryAttemptTotal;
    AppLogger.info(
      '[VPN_SM] {"event":"recovery_success","source":"$source","failure_type":"${_lastRecoveryFailureType?.name ?? "none"}","attempts_total":$_recoveryAttemptTotal,"successes":$_recoverySuccessCount,"success_rate":${successRate.toStringAsFixed(3)}}',
    );
    _resetRecoveryBackoff(logReason: 'recovery_success');
  }

  void _resetRecoveryBackoff({required String logReason}) {
    if (_recoveryAttemptCount == 0 &&
        _scheduledReconnectAt == null &&
        _lastRecoveryFailureType == null) {
      return;
    }
    AppLogger.info(
      '[VPN_SM] {"event":"recovery_backoff_reset","reason":"$logReason"}',
    );
  ...
  Future<void> _respectReconnectDelay(_VpnOperation op) async {
    final scheduled = _scheduledReconnectAt;
    Duration remaining;
    if (scheduled != null) {
      remaining = scheduled.difference(DateTime.now());
    ...
    AppLogger.info(
      '[VPN_SM] {"event":"reconnect_backoff","wait_ms":${remaining.inMilliseconds},"attempt":$_recoveryAttemptCount}',
    );
    ...
    if (state.desiredOn) {
      switch (state.status) {
        ...
        case VpnStatus.reconnecting:
          ...
          if (state.status == VpnStatus.reconnecting) {
  ...
  Future<_ResolvedVpnProfile?> _maybeRecoverConnectFailureWithFailover({
    required Object error,
  ...
  Future<void> _verifyConnectedTunnel({
    required _VpnOperation op,
    required VpnProtocol effectiveProtocol,
    required ChannelVpnService service,
  }) async {
    state = state.copyWith(connectPhase: ConnectPhase.verifyingConnection);
    _transitionTo(
      VpnStatus.verifying,
      trigger: VpnTransitionTrigger.connectVerificationStarted,
      operationId: op.id,
    );
    ...
    VpnHealthSnapshot? lastSnapshot;
    ...
    final failure = lastSnapshot;
    if (failure == null) {
      throw VpnServiceException(
        'connect_verification_failed',
        'Tunnel verification did not return native health data.',
      );
    }
    if (!failure.interfaceUp) {
      throw VpnServiceException(
        'connect_verification_failed',
        'Tunnel verification failed because the interface is not up.',
      );
    }
    if (!failure.routePresent || !failure.policyRoutingPresent) {
      throw VpnServiceException(
        'connect_verification_failed',
        'Tunnel verification failed because policy routing is missing.',
      );
    }
    throw VpnServiceException(
      'connect_verification_failed',
  ...
  Future<void> _runConnectFlow() async {
    final recoveryFlow =
        state.status == VpnStatus.reconnecting || state.reconnectPending;
    ...
    try {
      ...
      late _ResolvedVpnProfile resolvedProfile;
      try {
        resolvedProfile = await _resolveVpnProfile(
          op: op,
      ...
      final adapter = _ref.read(vpnAdapterProvider);
      late final VpnConnectionResult connectionResult;
      while (true) {
        _throwIfCancelled(op);
        try {
          connectionResult = await adapter
              .connect(resolvedProfile.profile)
          ...
          resolvedProfile = recoveredProfile;
        } catch (error) {
          final recoveredProfile =
              await _maybeRecoverConnectFailureWithFailover(
            error: error,
      ...
      _throwIfCancelled(op);
      final nextStatus = connectionResult.status;
      ...
      if (!isMockVpn && service is ChannelVpnService) {
        await _verifyConnectedTunnel(
          op: op,
          effectiveProtocol: plan.effective,
          service: service,
        );
      ...
      _transitionTo(
        VpnStatus.connected,
        trigger: VpnTransitionTrigger.connectVerificationSucceeded,
        operationId: op.id,
      );
      if (nextStatus == VpnStatus.connected) {
        state = state.copyWith(
        ...
        if (recoveryFlow) {
          _recordRecoverySuccess(source: 'connect_success');
        } else {
          _resetRecoveryBackoff(logReason: 'manual_connect_success');
        }
        _startRateSimulation();
        _safeFireAndForget(
      ...
      _debugTrace('[VPN_DIAG] connect op CANCELLED: ${e.reason}');
      _transitionTo(
        VpnStatus.error,
        trigger: VpnTransitionTrigger.connectOperationCancelled,
        operationId: op.id,
  ...
  Future<_ResolvedVpnProfile> _resolveVpnProfile({
    required _VpnOperation op,
    ...
    Future<VpnProfile> fetchProfile({
      required int? requestedDeviceId,
      required String? requestedServerId,
    }) {
      return api
          .fetchVpnProfile(
    ...
    try {
      ...
      var requestedServerId = state.selectedServerId;
      var attemptedDeviceRecovery = false;
      var attemptedServerReset = false;
      var attemptedPinnedRegionRetry = false;
      ...
      while (true) {
        try {
          ...
          if (!attemptedDeviceRecovery && _isDeviceLimitCode(code)) {
            attemptedDeviceRecovery = true;
            final recoveredDeviceId = await _recoverSingleDeviceSlotId(
              api: api,
              identity: identity,
              requestedDeviceId: requestedDeviceId,
            );
            if (recoveredDeviceId != null &&
                recoveredDeviceId != requestedDeviceId) {
              AppLogger.warning(
                '[VPN_SM] {"event":"profile_retry_with_recovered_device_id","device_id":$recoveredDeviceId}',
              );
              requestedDeviceId = recoveredDeviceId;
              _throwIfCancelled(op);
              continue;
            }
          }
          ...
          if (transientPinnedRegionFailure) {
            ...
            AppLogger.warning(
              '[VPN_SM] {"event":"profile_retry_pinned_region","server_id":"${requestedServerId ?? ""}","code":"$code"}',
            );
            _throwIfCancelled(op);
            continue;
          }
          if (!attemptedServerReset && staleSelectedServer) {
            attemptedServerReset = true;
            final staleServerId = requestedServerId;
            AppLogger.warning(
      ...
      return _ResolvedVpnProfile(
        profile: _resolvedAdapterProfile(
          profile: profile,
          effectiveProtocol: effectiveProtocol,
          nativeProfile: nativeProfile,
        ),
  ...
  VpnProfile _resolvedAdapterProfile({
    ...
    final json = <String, dynamic>{
      ...
      if ((profile.deviceName ?? '').trim().isNotEmpty)
        'device_name': profile.deviceName,
      if ((profile.deviceType ?? '').trim().isNotEmpty)
        'device_type': profile.deviceType,
      'protocol': vpnProtocolStorageValue(effectiveProtocol),
      'server_id': profile.serverId,
      'server_location': profile.serverLocation,
      if (profile.issuedAt != null)
        'issued_at': profile.issuedAt!.toIso8601String(),
      if (profile.expiresAt != null)
        'expires_at': profile.expiresAt!.toIso8601String(),
      'peer_registered': profile.peerRegistered,
      if ((profile.registrationStatus ?? '').trim().isNotEmpty)
        'registration_status': profile.registrationStatus,
      'dns': <String, dynamic>{
        'servers': profile.dnsServers,
        'ad_malware_blocking': profile.adMalwareBlocking,
        'enforcement': profile.dnsEnforcement,
      },
      'kill_switch': <String, dynamic>{
        'mode': profile.killSwitchMode,
        'enforcement': profile.killSwitchEnforcement,
      },
      ...nativeProfile,
  ...
  Future<void> _notifyBackendDisconnected() async {
    ...
    _debugTrace('[VPN_VERIFY] userPlanProvider invalidated (disconnect)');
  }
  ...
  Future<void> _notifyBackendConnected({required VpnProtocol protocol}) async {
    ...
    _ref.invalidate(userPlanProvider);
    _debugTrace('[VPN_VERIFY] userPlanProvider invalidated (connect)');
  }
  ...
  Future<void> _captureMetricsSnapshot({
    ...
    try {
      ...
      if (snapshot == null || snapshot.isEmpty) {
        _debugTrace('[VPN_VERIFY] metrics snapshot empty for event=$event');
        return;
      }
      _debugTrace(
  ...
  Future<VpnStatus> _disconnectRuntime({
    ...
    if (inFlight != null) {
      if (kDebugMode) {
        _debugTrace('[VPN_DIAG] disconnect runtime reused source=$source');
      }
      return inFlight;
    }
    ...
    future = adapter.disconnect().then((_) {
      _lastAdapterStatus = VpnStatus.disconnected;
  ...
  bool _transitionTo(
    ...
    if (_statusSupportsHealthMonitoring(current) &&
        !_statusSupportsHealthMonitoring(next)) {
      _stopRateSimulation();
      _stopHealthMonitor();
    }
    if (!_statusSupportsHealthMonitoring(current) &&
  ...
  Future<void> _pollTrafficRates() async {
    ...
    try {
      ...
      if (!mounted || _disposed || !_statusIsConnectedLike(state.status)) {
        return;
      }
  ...
  Future<void> _maybeTriggerDataPlaneFailover({
    ...
    if (!state.desiredOn || !_statusIsConnectedLike(state.status)) return;
    final now = DateTime.now();
  ...
  void dispose() {
    ...
    _adapterStatusSubscription = null;
    _stopHealthMonitor();
    _rateTimer?.cancel();
    _rateTimer = null;
    super.dispose();
...
class _ResolvedVpnProfile {
  const _ResolvedVpnProfile({
    required this.profile,
    required this.nativeProfile,
  });

  final VpnProfile profile;
  final Map<String, dynamic> nativeProfile;
}
...
extension VpnStatePresentation on VpnState {
  ...
  String statusText({bool includeEllipsis = false}) {
    ...
    return switch (status) {
      ...
      VpnStatus.degraded => 'Connection degraded',
      VpnStatus.connecting => includeEllipsis ? 'Connecting…' : 'Connecting',
      VpnStatus.verifying => includeEllipsis ? 'Verifying…' : 'Verifying',
      VpnStatus.reconnecting =>
        includeEllipsis ? 'Reconnecting…' : 'Reconnecting',
      VpnStatus.disconnecting =>
  ...
  IconData get statusIcon {
    ...
    return switch (status) {
      ...
      VpnStatus.degraded => Icons.warning_amber_rounded,
      VpnStatus.connecting => Icons.sync,
      VpnStatus.verifying => Icons.verified_user,
      VpnStatus.reconnecting => Icons.sync_problem,
