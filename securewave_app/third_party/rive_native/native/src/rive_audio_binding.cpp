...
{
    return (WasmPtr)rive::AudioEngine::MakeAndStore(numChannels, sampleRate)
        .release();
}
...
void stopAudio(WasmPtr enginePtr)
{
    ...
    if (engine)
    {
        ...
        // Pass nullptr to stop all sounds
        engine->stop(nullptr);
#endif
    }
#endif
...
{
#ifdef WITH_RIVE_AUDIO
    return rive::AudioEngine::MakeAndStore(numChannels, sampleRate).release();
#else
    return nullptr;
#endif
}
...
EXPORT void stopAudio(rive::AudioEngine* engine)
{
#ifdef WITH_RIVE_AUDIO
    if (engine)
    {
