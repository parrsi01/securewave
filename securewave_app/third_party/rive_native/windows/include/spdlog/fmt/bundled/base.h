...
struct is_std_string_like<
    T,
    void_t<decltype(std::declval<T>().find_first_of(typename T::value_type(),
                                                    0))>>
...
public:
    ...
    template <typename S,
              FMT_ENABLE_IF(detail::is_std_string_like<S>::value&& std::
                                is_same<typename S::value_type, Char>::value)>
...
using basic_format_context =
    conditional_t<std::is_same<OutputIt, appender>::value,
                  context,
...
public:
    ...
    {
        data_ =
            (data_ & ~align_mask) | (static_cast<unsigned>(a) << align_shift);
    }
    ...
    {
        data_ =
            (data_ & ~width_mask) | (static_cast<unsigned>(w) << width_shift);
    }
...
struct has_to_string_view<
    T,
    void_t<decltype(detail::to_string_view(std::declval<T>()))>>
    : std::true_type
{};
...
FMT_CONSTEXPR void init_static_named_arg(named_arg_info<Char>*,
                                         int& arg_index,
                                         int&)
{
    ++arg_index;
...
struct use_format_as<
    T,
    bool_constant<std::is_arithmetic<format_as_result<T>>::value>>
    : std::true_type
{};
template <typename T>
struct use_format_as_member<
    T,
    bool_constant<std::is_arithmetic<format_as_member_result<T>>::value>>
    : std::true_type
{};
...
using stored_type_constant =
    std::integral_constant<type,
                           Context::builtin_types || TYPE == type::int_type
                               ? TYPE
                               : type::custom_type>;
...
public:
    ...
    FMT_CONSTEXPR auto on_format_specs(int id,
                                       const Char* begin,
                                       const Char* end) -> const Char*
    {
        ...
        for (int bracket_count = 0;
             begin != end && (bracket_count > 0 || *begin != '}');
             ++begin)
        {
            if (*begin == '{')
                ++bracket_count;
...
template <
    ...
    FMT_ENABLE_IF(
        is_back_insert_iterator<OutputIt>::value &&
        !has_back_insert_iterator_container_append<OutputIt, InputIt>::value)>
FMT_CONSTEXPR20 auto copy(InputIt begin, InputIt end, OutputIt out) -> OutputIt
{
...
private:
    ...
    static void format_custom(void* arg,
                              parse_context<char_type>& parse_ctx,
                              Context& ctx)
    {
        auto f = Formatter();
...
using arg_t = conditional_t<NUM_ARGS <= max_packed_args,
                            value<Context>,
...
FMT_API void vformat_to(buffer<char>& buf,
                        string_view fmt,
                        format_args args,
                        locale_ref loc = {});
...
public:
    ...
    /// arguments.
    constexpr basic_format_args(const format_arg* args,
                                int count,
...
public:
    ...
    FMT_CONSTEVAL FMT_ALWAYS_INLINE fstring(const char (&s)[N]) : str(s, N - 1)
    {
        ...
        static_assert(FMT_USE_CONSTEVAL && sizeof(s) != 0,
                      "FMT_ENFORCE_COMPILE_STRING requires format strings to "
                      "use FMT_STRING");
#endif
    }
    template <typename S,
...
template <typename... T>
using vargs =
    detail::format_arg_store<context,
                             sizeof...(T),
                             detail::count_named_args<T...>(),
