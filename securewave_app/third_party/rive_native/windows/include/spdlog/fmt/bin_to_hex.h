    ...
    size_t size_per_line = 32)
{
    ...
    return details::dump_info<Iter>(std::begin(container),
                                    std::end(container),
                                    size_per_line);
}
...
inline details::dump_info<typename std::span<Value, Extent>::iterator> to_hex(
    const std::span<Value, Extent>& container,
    size_t size_per_line = 32)
{
...
template <typename T> struct formatter<spdlog::details::dump_info<T>, char>
{
    ...
    template <typename It> void put_newline(It inserter, std::size_t pos) const
    {
#ifdef _WIN32
        *inserter++ = '\r';
#endif
        *inserter++ = '\n';
        ...
        if (put_positions)
        {
            spdlog::fmt_lib::format_to(inserter,
                                       SPDLOG_FMT_STRING("{:04X}: "),
                                       pos);
        }
    }
};
