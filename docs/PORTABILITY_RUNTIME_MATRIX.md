...
4. The tracked `static/downloads/securewave-linux-x64.tar.gz` contains an ARM
   ...
   proof despite the filename.
5. `static/downloads/manifest.json` now keeps the Linux x64 `.deb`, x64
   AppImage, and x64 portable tarball as `coming_soon`. The download route must
   honor that declared status and must not recommend the ARM64 zip to a Linux
   x64 user-agent.
...
| Linux x64 Debian/Ubuntu/systemd VM | Linux x64 `.deb`, x64 AppImage, and x64 portable tarball are `coming_soon`; tracked `securewave-linux-x64.tar.gz` is present but contains an ARM aarch64 binary and is intentionally withheld by manifest status. | Blocked for x64 from current tracked artifacts. | Blocked. No valid x64 artifact proof and no clean x86_64 runtime proof. | Future `.deb` install should require admin once; current x64 `.deb` artifact is absent. | Unknown for a valid x64 release artifact. Current source would use direct `pkexec` unless the helper model is restored and packaged. | Same source paths as Linux ARM64 if built correctly: WireGuard and OpenVPN implemented; IKEv2 unsupported. | Blocked. | Manifest marks x64 `.deb`, AppImage, and tarball `coming_soon`; download detection does not fall back to ARM64 artifacts for Linux x64. | Valid x64 artifact, x64 checksums, clean x86_64 VM install, helper/systemd proof, UI launch, WireGuard/OpenVPN connect/disconnect/routing proof, and uninstall proof. |
...
- Download route detection was checked in the project virtualenv. A Linux x64
  user-agent must receive no recommendation until a true x64 artifact is
  available; it must not receive the ARM64 portable zip as a fallback.
...
- Linux x64 tar claim, because the tracked tar contains an ARM64 binary and is
  intentionally withheld by manifest status.
