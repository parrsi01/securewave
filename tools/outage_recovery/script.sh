...
TARGET="${REPO_ROOT}/tools/outage_recovery/vps_console_recover_and_validate.sh"
if [[ ! -x "${TARGET}" ]]; then
  TARGET="${REPO_ROOT}/scripts/ops/hetzner_console_recover_and_validate.sh"
fi

if [[ ! -x "${TARGET}" ]]; then
  echo "Could not find executable recovery runner." >&2
  echo "Expected one of:" >&2
  echo "  ${REPO_ROOT}/tools/outage_recovery/vps_console_recover_and_validate.sh" >&2
  echo "  ${REPO_ROOT}/scripts/ops/hetzner_console_recover_and_validate.sh" >&2
  exit 1
fi
