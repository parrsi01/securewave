  ...
  for i in $(seq 1 20); do
    if grep -q 'Initialization Sequence Completed' "$OPENVPN_WORK/client.log" 2>/dev/null; then
      OPENVPN_TUN_IF="$(
        ...
        ' "$OPENVPN_WORK/client.log" 2>/dev/null
      )"
      if [[ -z "${OPENVPN_TUN_IF:-}" ]]; then
        OPENVPN_TUN_IF="$(ip -o link show | awk -F': ' '{print $2}' | grep -E '^tun[[:alnum:]_.-]*$' | tail -n1 || true)"
      fi
      # Force full-tunnel routes onto the self-test interface to avoid
      # ambiguous gateway resolution when server/client share one host.
      ip route replace 0.0.0.0/1 dev "$OPENVPN_TUN_IF" || true
      ip route replace 128.0.0.0/1 dev "$OPENVPN_TUN_IF" || true
...
password = os.environ.get("SW_EGRESS_TEST_PASSWORD", f"sw-egress-{int(time.time())}")
...
PY
  ...
  IKEV2_HELPER_PID="$!"
  disown "$IKEV2_HELPER_PID" 2>/dev/null || true
...
if [[ "$PROTO" == "openvpn" ]]; then
  if [[ "${OPENVPN_SELFTEST_AUTO:-1}" == "1" && "$(id -u)" -eq 0 ]]; then
    if start_openvpn_selftest; then
      TUNNEL_UP="true"
      TUN_IF="${OPENVPN_TUN_IF:-}"
    fi
  fi
