...
if git grep -n "BEGIN [A-Z ]*PRIVATE KEY" -- \
  ...
  git grep -n "BEGIN [A-Z ]*PRIVATE KEY" -- ':!securewave_app/ios/ThirdParty/**' ':!docs/reference/**' ':!scripts/pre-commit-hook.sh' ':!scripts/scan_git_history_for_secrets.sh' ':!scripts/secret_scan.sh' ':!artifacts/**' >&2 || true
  exit 14
fi
